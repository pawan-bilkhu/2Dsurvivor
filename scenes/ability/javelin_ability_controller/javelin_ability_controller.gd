...
var javelin_quantity: int = 10

var stats: WeaponStats

func _ready() -> void:
	stats = GameStats.get_weapon_stats_resource("javelin")
...
func _on_timer_timeout() -> void:
	...
	for i in javelin_quantity:
		...
		javelin_instance.global_position = player.marker_2d.global_position
...
func on_ability_upgrade_added(upgrade: AbilityUpgrade, current_upgrades: Dictionary) -> void:
	if upgrade.id == "javelin_rate":
		...
		additional_damage_percent = 1 + (current_upgrades["javelin_damage"]["z"] * 0.3)
